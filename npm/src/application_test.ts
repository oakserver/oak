...
import type { Data, Server, ServerConstructor } from "./types.d.js";
...
function setup(
...
): [ServerConstructor<NativeRequest>, denoShim.Response[]] {
  const responseStack: denoShim.Response[] = [];
  ...
  return [
    class MockNativeServer<AS extends State = Record<string, any>>
      ...
      constructor(
        _app: Application<AS>,
        private options: denoShim.Deno.ListenOptions | denoShim.Deno.ListenTlsOptions,
      ) {
...
test({
  ...
  async fn() {
    ...
    const actual = await app.handle(new denoShim.Request("http://localhost/index.html"));
    assertEquals(called, 1);
    assert(actual);
...
test({
  ...
  async fn() {
    ...
    const request = new denoShim.Request("http://localhost:8080/", {
      method: "POST",
      body: `{"a":"b"}`,
    });
    const conn = {
...
test({
  ...
  async fn() {
    const app = new Application();
    await assertThrowsAsync(async () => {
      await app.handle(new denoShim.Request("http://localhost/index.html"));
    }, TypeError);
    teardown();
  },
});
...
test({
  ...
  fn() {
    assertEquals(
      denoShim.Deno.inspect(new Application()),
