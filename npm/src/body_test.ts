...
test({
  ...
  async fn() {
    const rBody = "hello";
    const requestBody = new RequestBody(
      new denoShim.Request("http://localhost/index.html", {
        body: rBody,
        method: "POST",
        headers: {
...
test({
  ...
  async fn() {
    ...
    const actual = await new denoShim.Response(body.value).text();
...
test({
  ...
  async fn() {
    ...
    const textA = await new denoShim.Response(a.value).text();
    const textB = await new denoShim.Response(b.value).text();
