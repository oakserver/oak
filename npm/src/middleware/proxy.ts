...
export type Fetch = (input: denoShim.Request) => Promise<denoShim.Response>;
...
> {
  ...
  request?(req: denoShim.Request): denoShim.Request | Promise<denoShim.Request>;
  ...
  response?(res: denoShim.Response): denoShim.Response | Promise<denoShim.Response>;
}
...
>(
  ...
  const headers = new denoShim.Headers(ctx.request.headers);
  if (optHeaders) {
  ...
  let request = new denoShim.Request(url.toString(), init);
  if (reqFn) {
...
>(
  ...
  return async function proxy(ctx, next) {
    ...
    const { fetch = ({ ...denoShim, ...globalThis }).fetch } = options;
