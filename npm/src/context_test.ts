...
import { NativeRequest } from "./http_server_native.js";
import type { UpgradeWebSocketFn } from "./http_server_native.js";
import { Request as OakRequest } from "./request.js";
import { Response as OakResponse } from "./response.js";
import { cloneState } from "./structured_clone.js";
import type { UpgradeWebSocketOptions } from "./types.d.js";
...
let respondWithStack: (denoShim.Response | Promise<denoShim.Response>)[] = [];
let upgradeWebSocketStack: [denoShim.Request, UpgradeWebSocketOptions | undefined][] =
  [];

const mockWebSocket = {} as WebSocket;
const mockResponse = {} as denoShim.Response;
...
function createMockNativeRequest(
  ...
  const request = new denoShim.Request(url, requestInit);
  const requestEvent = {
    request,
    respondWith(r: denoShim.Response | Promise<denoShim.Response>) {
...
test({
  ...
  async fn() {
    ...
    const fixture = await denoShim.Deno.readFile("./fixtures/test.html");
...
test({
  ...
  fn() {
    ...
    assertEquals(
      denoShim.Deno.inspect(new Context(app, req, {}), { depth: 1 }),
