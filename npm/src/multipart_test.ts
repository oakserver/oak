...
import { FormDataFile, FormDataReader } from "./multipart.js";
import { equals, lookup, parse } from "./deps.js";
import { stripEol } from "./util.js";
...
function createBodyFile(
  ...
  const fileData = denoShim.Deno.readFileSync(filename);
...
test({
  ...
  async fn() {
    ...
    const actualFileData = await denoShim.Deno.readFile(actualItem.filename);
...
test({
  ...
  async fn() {
    ...
    const file = await denoShim.Deno.stat(actual.files[0].filename);
...
test({
  ...
  fn() {
    const body = createBody(fixture);
    assertEquals(
      denoShim.Deno.inspect(new FormDataReader(fixtureContentType, body)),
      `FormDataReader {}`,
    );
  },
});
