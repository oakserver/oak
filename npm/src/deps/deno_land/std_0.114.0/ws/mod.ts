...
import { readLong, readShort, sliceLongToBytes } from "../io/util.js";
import { crypto } from "../crypto/mod.js";
...
export interface WebSocket extends AsyncIterable<WebSocketEvent> {
  readonly conn: denoShim.Deno.Conn;
...
class WebSocketImpl implements WebSocket {
  ...
  private enqueue(frame: WebSocketFrame): Promise<void> {
    if (this._isClosed) {
      throw new denoShim.Deno.errors.ConnectionReset("Socket has already been closed");
    }
  ...
  private ensureSocketClosed(): void {
    ...
    try {
      ...
      rest.forEach((e) =>
        e.d.reject(
          new denoShim.Deno.errors.ConnectionReset("Socket has already been closed"),
        )
      );
    }
  }
 ...
 */
export function acceptable(req: { headers: denoShim.Headers }): boolean {
...
export async function acceptWebSocket(req: {
  ...
  if (acceptable(req)) {
    ...
    const newHeaders = new denoShim.Headers({
      Upgrade: "websocket",
      Connection: "Upgrade",
