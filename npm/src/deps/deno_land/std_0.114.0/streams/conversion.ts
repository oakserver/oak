 ...
 * ```
 */
export async function readAll(r: denoShim.Deno.Reader): Promise<Uint8Array> {
 ...
 * ```
 */
export function readAllSync(r: denoShim.Deno.ReaderSync): Uint8Array {
  const buf = new Buffer();
  buf.readFromSync(r);
  return buf.bytes();
}
 ...
 * ```
 */
export async function writeAll(w: denoShim.Deno.Writer, arr: Uint8Array) {
  let nwritten = 0;
 ...
 * ```
 */
export function writeAllSync(w: denoShim.Deno.WriterSync, arr: Uint8Array): void {
  let nwritten = 0;
  while (nwritten < arr.length) {
...
export async function copy(
  src: denoShim.Deno.Reader,
  dst: denoShim.Deno.Writer,
  options?: {
    bufSize?: number;
  },
): Promise<number> {
