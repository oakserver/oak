...
import { BufReader, BufWriter } from "../io/buffer.js";
import { copy, iterateReader } from "../streams/conversion.js";
import { TextProtoReader } from "../textproto/mod.js";
import { assert } from "../_util/assert.js";
import { Response, ServerRequest } from "./server_legacy.js";
import { STATUS_TEXT } from "./http_status.js";

const encoder = new TextEncoder();

export function emptyReader(): denoShim.Deno.Reader {
  return {
...
export function bodyReader(contentLength: number, r: BufReader): denoShim.Deno.Reader {
  let totalRead = 0;
  let finished = false;
...
export function chunkedBodyReader(h: denoShim.Headers, r: BufReader): denoShim.Deno.Reader {
  ...
  async function read(buf: Uint8Array): Promise<number | null> {
    ...
    if (line === null) throw new denoShim.Deno.errors.UnexpectedEof();
    // TODO(bartlomieju): handle chunk extension
    const [chunkSizeString] = line.split(";");
    const chunkSize = parseInt(chunkSizeString, 16);
    if (Number.isNaN(chunkSize) || chunkSize < 0) {
      throw new denoShim.Deno.errors.InvalidData("Invalid chunk size");
    }
    if (chunkSize > 0) {
...
export async function readTrailers(
  ...
  if (result == null) {
    throw new denoShim.Deno.errors.InvalidData("Missing trailer header.");
  }
  const undeclared = [...result.keys()].filter(
    (k) => !trailerNames.includes(k),
  );
  if (undeclared.length > 0) {
    throw new denoShim.Deno.errors.InvalidData(
      `Undeclared trailers: ${denoShim.Deno.inspect(undeclared)}.`,
    );
  }
  ...
  if (missingTrailers.length > 0) {
    throw new denoShim.Deno.errors.InvalidData(
      `Missing trailers: ${denoShim.Deno.inspect(missingTrailers)}.`,
    );
  }
  headers.delete("trailer");
}

function parseTrailer(field: string | null): denoShim.Headers | undefined {
  if (field == null) {
    return undefined;
  }
  const trailerNames = field.split(",").map((v) => v.trim().toLowerCase());
  if (trailerNames.length === 0) {
    throw new denoShim.Deno.errors.InvalidData("Empty trailer header.");
  }
  const prohibited = trailerNames.filter((k) => isProhibitedForTrailer(k));
  if (prohibited.length > 0) {
    throw new denoShim.Deno.errors.InvalidData(
      `Prohibited trailer names: ${denoShim.Deno.inspect(prohibited)}.`,
    );
  }
  return new denoShim.Headers(trailerNames.map((key) => [key, ""]));
}

export async function writeChunkedBody(
  w: BufWriter,
  r: denoShim.Deno.Reader,
) {
...
export async function writeTrailers(
  w: denoShim.Deno.Writer,
  headers: denoShim.Headers,
  trailers: denoShim.Headers,
) {
  ...
  if (prohibitedTrailers.length > 0) {
    throw new TypeError(
      `Prohibited trailer names: ${denoShim.Deno.inspect(prohibitedTrailers)}.`,
    );
  }
  ...
  if (undeclared.length > 0) {
    throw new TypeError(`Undeclared trailers: ${denoShim.Deno.inspect(undeclared)}.`);
  }
...
export async function writeResponse(
  ...
  const headers = r.headers ?? new denoShim.Headers();
...
export async function readRequest(
  ...
  if (headers === null) throw new denoShim.Deno.errors.UnexpectedEof();
