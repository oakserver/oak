...
import { Deferred, deferred, MuxAsyncIterator } from "../async/mod.js";
import {
  bodyReader,
  chunkedBodyReader,
  emptyReader,
...
import { _parseAddrFromStr } from "./server.js";
...
export class ServerRequest {
  ...
  #body?: denoShim.Deno.Reader = undefined;
  #finalized = false;
  ...
  async respond(r: denoShim.Response) {
...
export class Server implements AsyncIterable<ServerRequest> {
  #closing = false;
  #connections: denoShim.Deno.Conn[] = [];

  constructor(public listener: denoShim.Deno.Listener) {}
  ...
  close(): void {
    ...
    for (const conn of this.#connections) {
      try {
        ...
        if (!(e instanceof denoShim.Deno.errors.BadResource)) {
          throw e;
        }
      }
    }
  ...
  private trackConnection(conn: denoShim.Deno.Conn): void {
    this.#connections.push(conn);
  }

  private untrackConnection(conn: denoShim.Deno.Conn): void {
 ...
 */
export type HTTPOptions = Omit<denoShim.Deno.ListenOptions, "transport">;
...
export function serve(addr: string | HTTPOptions): Server {
  ...
  const listener = denoShim.Deno.listen(addr);
 ...
 * @deprecated
 */
export type HTTPSOptions = Omit<denoShim.Deno.ListenTlsOptions, "transport">;
...
export function serveTLS(options: HTTPSOptions): Server {
  const tlsOptions: denoShim.Deno.ListenTlsOptions = {
    ...options,
    transport: "tcp",
  };
  const listener = denoShim.Deno.listenTls(tlsOptions);
  return new Server(listener);
}
...
export interface Response {
  ...
  statusText?: string;
  headers?: denoShim.Headers;
  body?: Uint8Array | denoShim.Deno.Reader | string;
  trailers?: () => Promise<denoShim.Headers> | denoShim.Headers;
}
