...
import { concat, contentType, copyBytes, Status } from "./deps.js";
...
async function readRange(
  file: denoShim.Deno.Reader & denoShim.Deno.Seeker,
  range: ByteRange,
): Promise<Uint8Array> {
  let length = range.end - range.start + 1;
  assert(length);
  await file.seek(range.start, denoShim.Deno.SeekMode.Start);
...
export class MultiPartStream extends ReadableStream<Uint8Array> {
  ...
  constructor(
    file: (denoShim.Deno.Reader & denoShim.Deno.Seeker & denoShim.Deno.Closer) | Uint8Array,
    type: string,
    ranges: ByteRange[],
    size: number,
    boundary: string,
