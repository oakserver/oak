...
import { Status, STATUS_TEXT } from "./deps.js";
...
import { Key, Server, ServerConstructor } from "./types.d.js";
import { assert, isConn } from "./util.js";

export interface ListenOptionsBase extends denoShim.Deno.ListenOptions {
  secure?: false;
  signal?: AbortSignal;
}

export interface ListenOptionsTls extends denoShim.Deno.ListenTlsOptions {
...
export interface HandleMethod {
  ...
  (
    ...
    secure?: boolean,
  ): Promise<denoShim.Response | undefined>;
}
...
export class Application<AS extends State = Record<string, any>>
  ...
  #handleError(context: Context<AS>, error: any): void {
    ...
    const status: Status = context.response.status =
      denoShim.Deno.errors && error instanceof denoShim.Deno.errors.NotFound
        ? 404
  ...
  handle = (async (
    request: denoShim.Request,
    secureOrConn: denoShim.Deno.Conn | boolean | undefined,
  ...
  async listen(options: string | ListenOptions): Promise<void> {
    ...
    const { hostname, port } = listener.addr as denoShim.Deno.NetAddr;
    this.dispatchEvent(
