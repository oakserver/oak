...
import { assert, assertEquals, BufWriter, StringWriter } from "./test_deps.js";

import type { Application } from "./application.js";
import { Context } from "./context.js";
import { NativeRequest } from "./http_server_native.js";
import { ServerSentEvent, SSEStreamTarget } from "./server_sent_event.js";
...
const env = {
  ...
  response: undefined as (denoShim.Response | undefined),
};
...
function createMockNativeRequest(): NativeRequest {
  ...
  return new NativeRequest({
    request: new denoShim.Request("http://localhost:8000/"),
    async respondWith(p: denoShim.Response | Promise<denoShim.Response>) {
...
test({
  ...
  async fn() {
    ...
    const sse = new SSEStreamTarget(
      context,
      {
        headers: new denoShim.Headers([["X-Oak", "test"], ["Cache-Control", "special"]]),
      },
    );
    await request.respond(await context.response.toDomResponse());
    await sse.close();
...
test({
  ...
  fn() {
    ...
    const context = new Context(createMockApp(), request, {});
    assertEquals(
      denoShim.Deno.inspect(new SSEStreamTarget(context)),
      `SSEStreamTarget {\n  "#closed": false,\n  "#context": Context {\n  app: EventTarget {
    [Symbol()]: {
      assignedSlot: false,
