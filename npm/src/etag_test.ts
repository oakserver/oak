...
import { calculate, factory, ifMatch, ifNoneMatch } from "./etag.js";
...
test({
  ...
  async fn() {
    const fixture: denoShim.Deno.FileInfo = {
      isFile: true,
      isDirectory: false,
      isSymlink: false,
      size: 1024,
...
test({
  ...
  async fn() {
    const { context } = setup();
    let file: denoShim.Deno.File;
    async function next() {
      file = await denoShim.Deno.open("./fixtures/test.jpg", {
        read: true,
      });
