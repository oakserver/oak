...
async function calcEntityTag(entity: string | Uint8Array): Promise<string> {
  ...
  const hash = base64.encode(await denoShim.crypto.subtle.digest("SHA-1", entity))
    .substring(0, 27);

  return `"${entity.length.toString(16)}-${hash}"`;
}

function fstat(file: denoShim.Deno.File): Promise<denoShim.Deno.FileInfo | undefined> {
  if ("fstat" in Deno) {
    // deno-lint-ignore no-explicit-any
    return (denoShim.Deno as any).fstat(file.rid);
  }
...
export function getEntity<S extends State = Record<string, any>>(
  context: Context<S>,
): Promise<string | Uint8Array | denoShim.Deno.FileInfo | undefined> {
  const { body } = context.response;
  if (body instanceof denoShim.Deno.File) {
    return fstat(body);
  }
