...
import { BufReader, ReadLineResult } from "./buf_reader.js";
import { getFilename } from "./content_disposition.js";
import { equals, extension, writeAll } from "./deps.js";
...
async function* parts(
  ...
  async function getFile(contentType: string): Promise<[string, denoShim.Deno.File]> {
    const ext = extension(contentType);
    if (!ext) {
      throw new httpErrors.BadRequest(`Invalid media type for part: ${ext}`);
    }
    if (!outPath) {
      outPath = await denoShim.Deno.makeTempDir();
    }
    const filename = `${outPath}/${await getRandomFilename(prefix, ext)}`;
    const file = await denoShim.Deno.open(filename, { write: true, createNew: true });
  ...
  while (true) {
    ...
    if (contentType) {
      ...
      let byteLength = 0;
      let file: denoShim.Deno.File | undefined;
          ...
          if (file) {
            ...
            if (bytesDiff) {
              const originalBytesSize = await file.seek(
                -bytesDiff,
                denoShim.Deno.SeekMode.Current,
              );
...
export class FormDataReader {
  #body: denoShim.Deno.Reader;
  #boundaryFinal: Uint8Array;
  #boundaryPart: Uint8Array;
  #reading = false;

  constructor(contentType: string, body: denoShim.Deno.Reader) {
  ...
  async read(options: FormDataReadOptions = {}): Promise<FormDataBody> {
    ...
    try {
      ...
      if (err instanceof denoShim.Deno.errors.PermissionDenied) {
