...
import { calculate, FileInfo, ifNoneMatch } from "./etag.js";
...
async function exists(path: string): Promise<boolean> {
  try {
    return (await denoShim.Deno.stat(path)).isFile;
  } catch {
    return false;
  }
}
...
async function getEntity(
  path: string,
  mtime: number,
  stats: denoShim.Deno.FileInfo,
  maxbuffer: number,
  response: Response,
): Promise<[Uint8Array | denoShim.Deno.File, Uint8Array | FileInfo]> {
  let body: Uint8Array | denoShim.Deno.File;
  let entity: Uint8Array | FileInfo;
  const file = await denoShim.Deno.open(path, { read: true });
...
async function sendRange(
  response: Response,
  body: Uint8Array | denoShim.Deno.File,
  range: string,
  size: number,
) {
  ...
  if (ranges.length === 1) {
    ...
    if (body instanceof Uint8Array) {
      ...
      await body.seek(byteRange.start, denoShim.Deno.SeekMode.Start);
...
export async function send(
  ...
  let stats: denoShim.Deno.FileInfo;
  try {
    stats = await denoShim.Deno.stat(path);
    ...
    if (err instanceof denoShim.Deno.errors.NotFound) {
  ...
  let body: Uint8Array | denoShim.Deno.File | null = null;
