...
export interface RouterContext<P extends RouteParams = RouteParams>
  extends Context {
  params?: P;
  router: Router;
}

export interface RouterMiddleware<P extends RouteParams = RouteParams> {
  (context: RouterContext<P>, next: () => Promise<void>): Promise<void> | void;
}
    ...
    for (let i = 0; i < captures.length; i++) {
      if (this.paramNames[i]) {
        ...
        params[this.paramNames[i].name] = capture
          ? safeDecodeURIComponent(capture)
          : capture;
      }
    }
    return params;
